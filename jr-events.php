...
if (!function_exists("jr_events_refresh")) {
function jr_events_refresh() {
update_option("jr_submitted_events", "0");
}
}
...
update_option("jr_submitted_events", "1");
wp_schedule_single_event(time()+172800, 'jr_events_refresh'); 
} else {
...
if (get_option("jr_events_link_personal")=="") {
$content = curl_get_contents("http://www.jakeruston.co.uk/p_pluginslink4.php");

update_option("jr_events_link_personal", $content);
}
}
...
$linkper=utf8_decode(get_option('jr_events_link_personal'));

if (get_option("jr_events_link_newcheck")=="") {
$pieces=explode("</a>", get_option('jr_events_links_choice'));
...
update_option("jr_events_links_choice", $new);
update_option("jr_events_link_newcheck", "444");
}

if (get_option("jr_submitted_events")=="0") {
$pname="jr_events";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_events", "1");
...
wp_schedule_single_event(time()+172800, 'jr_events_refresh'); 
} else if (get_option("jr_submitted_events")=="") {
$pname="jr_events";
$url=get_bloginfo('url');
$current=get_option("jr_events_links_choice");
...
$eventsdisp .= "<p style='font-size:x-small'>Events Plugin created by ".$linkper." - ".get_option('jr_events_links_choice')."</p>";
}
